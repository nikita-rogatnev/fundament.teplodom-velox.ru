...
module.exports = () => {
    return gulp.task('main:build-dev', function (cb) {
        ...
        tars.options.watch.isActive = this
            .seq
            .slice(-1)[0] === 'dev'
            ? true
            : false; // eslint-disable-line no-invalid-this

        runSequence('service:clean', [
            'images:minify-svg', 'images:raster-svg'
        ], [
            'css:make-sprite-for-svg', 'css:make-fallback-for-svg', 'css:make-sprite', 'images:make-symbols-sprite'
        ], [
            'css:compile-css',
