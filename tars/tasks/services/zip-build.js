...
module.exports = () => {
    return gulp.task('service:zip-build', done => {
        ...
        if (tars.config.useArchiver) {
            ...
            const name = tars.packageInfo.name === 'awesome_project'
                ? 'build'
                : tars
                    .packageInfo
                    .name
            ...
            return gulp
                .src(`${tars.options.build.path}**`, {base: tars.options.build.path})
                .pipe(plumber({
                    errorHandler(error) {
                ...
                .pipe(notifier.success('Zip-archive\'s been created'));
        }
