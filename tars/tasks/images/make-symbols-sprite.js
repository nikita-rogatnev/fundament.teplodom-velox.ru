        ...
        if (tars.config.svg.active && tars.config.svg.workflow === 'symbols') {
            return gulp
                ...
                .pipe(tars.require('gulp-svg-symbols')({
                    templates: [
                        `${tars.root}/tasks/images/helpers/svg-symbols.svg`, `${tars.root}/tasks/images/helpers/symbols-data-template.js`
                    ],
                    transformData: (svg, defaultData) => {
                        return {id: defaultData.id, width: svg.width, height: svg.height, name: svg.name};
                    }
                }))
                .pipe(gulpif(/[.]svg$/, rename(spritePath => {
                    spritePath.basename += tars.options.build.hash;
                })))
                .pipe(gulpif(/[.]svg$/, gulp.dest(readySymbolSpritePath)))
                .pipe(gulpif(/[.]js$/, gulp.dest('./dev/temp/')))
